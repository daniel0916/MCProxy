...
bool cConnection::HandleServerEntityMetadata(void)
{
	...
	/**if (EntityID == m_ServerEntityID)
	{
		...
		Packet.WriteBEInt(m_ClientEntityID);
		Packet.WriteVarUTF8String(Metadata);
		AString Pkt;
		Packet.ReadAll(Pkt);
		cByteBuffer ToClient(512);
		ToClient.WriteVarUTF8String(Pkt);
...
bool cConnection::HandleServerJoinGame(void)
{
	...
	else
	{
		...
		/**cByteBuffer RespawnPacket(512);  // Not needed?
		RespawnPacket.WriteByte(0x07);
		RespawnPacket.WriteBEInt(1);
		RespawnPacket.WriteByte(0);
		RespawnPacket.WriteByte(0);
		RespawnPacket.WriteVarUTF8String("default");
		AString RespawnPkt;
		RespawnPacket.ReadAll(RespawnPkt);
		cByteBuffer RespawnToServer(512);
		RespawnToServer.WriteVarUTF8String(RespawnPkt);
		CLIENTSEND(RespawnToServer);*/
...
{
	DataLog(a_Data, a_Size, "Received %d bytes from the CLIENT", a_Size);
	...
	switch (m_ClientState)
	{
		...
		case csEncryptedUnknown:
		{
			DataLog(a_Data, a_Size, "Decrypted %d bytes from the CLIENT", a_Size);
...
{
	DataLog(a_Data, a_Size, "Received %d bytes from the SERVER", a_Size);
	...
	switch (m_ServerState)
	{
		...
		case csEncryptedUnderstood:
		{
			...
			DataLog(a_Data, a_Size, "Decrypted %d bytes from the SERVER", a_Size);
			return DecodeServersPackets(a_Data, a_Size);
		}
		/**case csEncryptedUnknown:
		{
