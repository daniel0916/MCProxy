...
void cAuthenticator::Authenticate(const AString & a_UserName, const AString & a_ServerHash)
{
	...
	cCSLock LOCK(m_CS);
	m_Queue.push_back(cUser(a_UserName, a_ServerHash));
	m_QueueNonempty.Set();
}
...
void cAuthenticator::Start()
{
	...
	m_ShouldAuthenticate = cServer::Get()->m_ShouldAuthenticate;
...
void cAuthenticator::Execute(void)
{
	for (;;)
	{
		...
		{
			cServer::Get()->KickUser(NewUserName, "Failed to authenticate account!");
		}
	}  // for (-ever)
}
